...
contract ApeStrapper is AccessControl, Pausable, ReentrancyGuard {
  ...
  event ApeList(
    ...
    address indexed apeAddress,
    uint256 apeAllocations
  );
  ...
  function approveContract() external onlyRole(APE_ROLE) {
    // Checks
    if (tx.origin != msg.sender) revert ContractsNotAllowed(tx.origin, msg.sender);
  ...
  function setApeAllocationList(
  ...
  ) external whenNotPaused onlyRole(APE_ADMIN_ROLE) {
    ...
    if (initialized) _clearApeAllocationList();
  ...
  function revokeContractCreatorAccess()
    external
  ...
  function calculatePortion(uint256 _contribution)
    public
    pure
    returns (uint256 _portion)
  {
  ...
  function _sendNanners(address _recipient, uint256 _amount) private {
    ...
    // Interactions
    (bool _success, bytes memory _error) = payable(_recipient).call{
      value: _amount
    }("");
    if (!_success) {
